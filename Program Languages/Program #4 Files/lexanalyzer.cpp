    ...
    for (int i = 0; i < readfile.length(); i++) {
        ...
        if (readfile[i] == ' ' || readfile[i] == '\t')
        {
            if (i == 0)
            {
                ...
                {
                    temp = temp + readfile[i];
                    i++;
                }
                p = make_pair(temp, categoryType::INDENT);
            ...
            p = make_pair(temp, categoryType::NUMERIC_LITERAL);
            tokenInfo[lineCount].push_back(p);
            i--;
        }
            ...
            if (temp == "print" || temp == "elif" || temp == "if" || temp == "else" || temp == "while" || temp == "int" || temp == "input") // if keyword
                p = make_pair(temp, categoryType::KEYWORD);

            else if (temp == "and" || temp == "or" || temp == "not") // if logical operator
                p = make_pair(temp, categoryType::LOGICAL_OP);
            else // if neither of above then identifier
                p = make_pair(temp, categoryType::IDENTIFIER);
            ...
            {
                temp = temp + readfile[i + 1];
                p = make_pair(temp, categoryType::RELATIONAL_OP);
                tokenInfo[lineCount].push_back(p);
                i++;
                continue;
            }
        ...
        { //if arithmetic op
            ...
            p = make_pair(temp, categoryType::COMMENT);
            tokenInfo[lineCount].push_back(p);
        }
        else if (readfile[i] == '\'') //if single quote
        {
            ...
            temp = temp + '\'';
            p = make_pair(temp, categoryType::STRING_LITERAL);
            tokenInfo[lineCount].push_back(p);
        }
